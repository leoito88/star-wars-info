...
const obtenerVehicles = async (page) => {
    try {
        const response = await fetch(`https://swapi.dev/api/vehicles/?page=${page}`);
        const data = await response.json();
        createCards(data.results);
        countDiv.textContent = `Vehículos encontrados: ${data.count}`;
...
obtenerVehicles(currentPage);

const createCards = async (vehicles) => {
    containercard.innerHTML = ""; // Limpiar el contenedor antes de agregar las tarjetas

    for (let vehicle of vehicles) {
        const { name, model, manufacturer, cost_in_credits, length, crew, max_atmosphering_speed, passengers, cargo_capacity, consumables, starship_class} = vehicle;
        ...
        containercard.innerHTML += `<div class="card">
            <div class="card-body">
                ...
                <p><span>Clase de Vehículo:</span> ${starship_class}</p>
            </div>
        </div>`;
    }
};
